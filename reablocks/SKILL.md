...
- **Unify Theme** — design-system theme built on a three-level token architecture (primitives → semantic → Tailwind), synced with the Unify Figma library. Available in two flavors:
  ...
  - **Production / Figma-synced**: the single-file [`themeUnify.ts`](./unify-theme/SKILL.md#download-the-pre-built-typescript-theme) download (every component theme + composed `ReablocksTheme`) plus CSS layers (`root.css` / `dark.css` / `light.css` / `tw.css`) generated by the Reablocks Figma Plugin. This is what powers per-component detail tokens like `--buttons-details-height-core-icon-lg`.
